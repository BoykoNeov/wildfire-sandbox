...
| Merging, retirement, loops, pockets | Grid-assisted, not FARSITE transcriptions (plan §D6–§D8). Each cell records the first front that **lit** it; a marker stepping onto another front's ground stops, so fires that meet weld. A front is retired once every marker is against a permanent wall or nothing burnable is left within two cells of it — byte-identical output to never retiring, on every preset's hour. A self-crossing front keeps its largest loop, drops the folded-over ears, and keeps any loop with unburned fuel inside it as an **inward-burning** front, so a pocket the fire closes around burns in from its rim rather than being left as a hole (a slow 100-cell patch in wind-driven grass: 60 cells left unburned before, 0 now, as on the raster). |
